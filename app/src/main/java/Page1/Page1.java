...
public class Page1 extends AppCompatActivity implements View.OnClickListener, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void isSecondMain(){
        ...
        while(iCursor.moveToNext()){
            ...
            String  endDate = iCursor.getString(iCursor.getColumnIndex("enddate"));
            list.add(id);
        ...
        if( list.size() > 0) {
            int myear2 = Integer.parseInt(end_Date.substring(0, 4));
            int mmonth2 = Integer.parseInt(end_Date.substring(4, 6));
            int mday2 = Integer.parseInt(end_Date.substring(6));
            ...
            if (today > ddayend) {
                ...
                text_dday.setText("D-" + second_date);

                if (second_date.equals("0")) {
                    text_mytrip.setVisibility(View.GONE);
                    text_dday.setText("지금은 여행 중");
                }

                if (second_date.contains("-")) {
                    text_mytrip.setVisibility(View.GONE);
                    text_dday.setText("지금은 여행 중");
                }
            }
